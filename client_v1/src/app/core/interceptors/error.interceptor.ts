...
function friendlyMessage(err: HttpErrorResponse): string {
  ...
  switch (err.status) {
    case 0:    return 'No response from server. Check your connection.';
    case 400:  return serverMsg ?? 'Bad request.';
    case 401:  return serverMsg ?? 'Session expired. Please sign in again.';
    case 403:  return serverMsg ?? 'You do not have permission to do that.';
    ...
    case 422:  return serverMsg ?? 'Validation failed. Check the form fields.';
