...
export async function getDashboardStats(
  ...
  if (requesterRole !== "manager" && requesterRole !== "admin") {
    throw ApiError.forbidden(
      "Only managers or admins can access the dashboard",
    );
  }
    ...
    Timesheet.aggregate<{ totalAmount: number }>([
      ...
      { $match: { "entries.isBillable": true } },
      {
        $group: {
          _id: null,
          totalAmount: {
            $sum: { $multiply: ["$entries.hours", "$user.hourlyRate"] },
          },
        },
      },
    ]),
    ...
    Timesheet.aggregate<PendingItem>([
      ...
      { $unwind: { path: "$user", preserveNullAndEmptyArrays: true } },
      {
        $project: {
          ...
          periodStart: {
            $dateToString: { format: "%Y-%m-%d", date: "$periodStart" },
          },
          periodEnd: {
            $dateToString: { format: "%Y-%m-%d", date: "$periodEnd" },
          },
          totalHours: 1,
          submittedAt: {
            $dateToString: {
  ...
  return {
    ...
    hours: {
      totalThisMonth: hoursRow.totalHours,
      billableThisMonth: hoursRow.billableHours,
    },
